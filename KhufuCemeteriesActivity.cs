...
[Activity(Label = "@string/khufu_cemeteries_title")]
public sealed class KhufuCemeteriesActivity : Activity
{
    private readonly LocalTourVoiceService _voiceService = LocalTourVoiceService.Shared;
    private readonly OnDeviceTourGuideService _chatService = new(TourGuideStop.KhufuCemeteries);
    ...
    protected override void OnCreate(Bundle? savedInstanceState)
    {
        ...
        var guide = GetString(Resource.String.khufu_cemeteries_message);

        FindViewById<TextView>(Resource.Id.sphinx_station_eyebrow_text)!
            .SetText(Resource.String.khufu_cemeteries_eyebrow);
        FindViewById<TextView>(Resource.Id.sphinx_station_title_text)!
            .SetText(Resource.String.khufu_cemeteries_title);
        ...
        {
            _voiceService.Stop();
            var destination = HasPriorityPassTicket() || HasAccessPassTicket()
                ? typeof(AccessPassRefreshmentsActivity)
                : typeof(TripProgressActivity);
        ...
        _conversation!.SetText(Resource.String.khufu_cemeteries_chat_welcome);
