...
public sealed class MenkaureStationActivity : Activity
{
    private readonly LocalTourVoiceService _voiceService = LocalTourVoiceService.Shared;
    private readonly OnDeviceTourGuideService _chatService = new(TourGuideStop.KingMenkaure);
    ...
    protected override void OnCreate(Bundle? savedInstanceState)
    {
        ...
        SetContentView(Resource.Layout.activity_menkaure_station);
        ...
        FindViewById<TextView>(Resource.Id.menkaure_station_next_button)!.Click += (_, _) =>
        {
            _voiceService.Stop();
            StartActivity(new Android.Content.Intent(this, typeof(MenkaureHistoryActivity)));
        };
        ...
        _chatPanel = FindViewById<LinearLayout>(Resource.Id.menkaure_station_chat_panel);
        _openChatButton = FindViewById<TextView>(Resource.Id.open_menkaure_station_chat_button);
        _openChatButton!.Click += (_, _) => SetChatExpanded(true);
        FindViewById<TextView>(Resource.Id.close_menkaure_station_chat_button)!.Click += (_, _) =>
            SetChatExpanded(false);
        _conversation = FindViewById<TextView>(Resource.Id.menkaure_station_chat_conversation);
        _chatInput = FindViewById<EditText>(Resource.Id.menkaure_station_chat_input);
        _sendButton = FindViewById<TextView>(Resource.Id.menkaure_station_chat_send_button);
    ...
    private async void Narrate()
    {
        ...
        if (await _voiceService.TrySpeakAsync(this, GetString(Resource.String.menkaure_station_narration)))
        {
            return;
        }
