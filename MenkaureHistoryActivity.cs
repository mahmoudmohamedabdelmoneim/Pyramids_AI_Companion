...
[Activity(Label = "@string/menkaure_history_title")]
public sealed class MenkaureHistoryActivity : Activity
{
    private readonly LocalTourVoiceService _voiceService = LocalTourVoiceService.Shared;
    private readonly OnDeviceTourGuideService _chatService = new(TourGuideStop.MenkaureHistory);
    ...
    protected override void OnCreate(Bundle? savedInstanceState)
    {
        ...
        SetContentView(Resource.Layout.activity_menkaure_history);

        var guide = GetString(Resource.String.menkaure_history_message);
        FindViewById<TextView>(Resource.Id.back_button)!.Click += (_, _) =>
        {
            _voiceService.Stop();
            Finish();
        };
        FindViewById<TextView>(Resource.Id.menkaure_history_replay_button)!.Click += (_, _) =>
            Narrate(guide);
        FindViewById<TextView>(Resource.Id.menkaure_history_next_button)!.Click += (_, _) =>
        {
            _voiceService.Stop();
            StartActivity(new Android.Content.Intent(this, typeof(MenkaureDepartureActivity)));
        };
        ...
        _chatPanel = FindViewById<LinearLayout>(Resource.Id.menkaure_history_chat_panel);
        _openChatButton = FindViewById<TextView>(Resource.Id.open_menkaure_history_chat_button);
        _openChatButton!.Click += (_, _) => SetChatExpanded(true);
        FindViewById<TextView>(Resource.Id.close_menkaure_history_chat_button)!.Click += (_, _) =>
            SetChatExpanded(false);
        _conversation = FindViewById<TextView>(Resource.Id.menkaure_history_chat_conversation);
        _chatInput = FindViewById<EditText>(Resource.Id.menkaure_history_chat_input);
        _sendButton = FindViewById<TextView>(Resource.Id.menkaure_history_chat_send_button);
