...
public sealed class KingKhufuJourneyActivity : Activity
{
    private readonly LocalTourVoiceService _voiceService = LocalTourVoiceService.Shared;
    private readonly OnDeviceTourGuideService _chatService = new(TourGuideStop.KhufuJourney);
    ...
    protected override void OnCreate(Bundle? savedInstanceState)
    {
        ...
        _guide = GetString(Resource.String.king_khufu_journey_message);
        var isPriorityPass = HasPriorityPassTicket();

        FindViewById<TextView>(Resource.Id.sphinx_transfer_eyebrow_text)!
            .SetText(Resource.String.king_khufu_journey_eyebrow);
        FindViewById<TextView>(Resource.Id.sphinx_transfer_title_text)!
        ...
        var inPageMap = FindViewById<GizaPlateauMapView>(Resource.Id.sphinx_transfer_map)!;
        inPageMap.ShowBusPills = !isPriorityPass;
        inPageMap.ContentDescription = GetString(Resource.String.king_khufu_journey_map_description);

        var expandMapButton = FindViewById<TextView>(Resource.Id.sphinx_transfer_expand_map_button)!;
        expandMapButton.ContentDescription = GetString(Resource.String.king_khufu_journey_map_description);
        expandMapButton.Click += (_, _) => OpenFullScreenMap();
        ...
        {
            _voiceService.Stop();
            StartActivity(new Android.Content.Intent(this, typeof(KingKhufuStationActivity)));
        };
        ...
        _conversation!.SetText(Resource.String.king_khufu_journey_chat_welcome);
