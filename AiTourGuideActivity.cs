...
{
    [Activity(Label = "@string/ai_tour_guide_title")]
    public class AiTourGuideActivity : Activity
    {
        private readonly OnDeviceTourGuideService _chatService = new();
        ...
        protected override void OnCreate(Bundle? savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.activity_ai_tour_guide);

            FindViewById<TextView>(Resource.Id.back_button)!.Click += (_, _) => Finish();
            FindViewById<TextView>(Resource.Id.next_stop_button)!.Click += (_, _) =>
            {
                _voiceService.Stop();
            ...
            _chatPanel = FindViewById<LinearLayout>(Resource.Id.ai_chat_panel);
            _openChatButton = FindViewById<TextView>(Resource.Id.open_ai_chat_button);
            _openChatButton!.Click += (_, _) => SetChatExpanded(true);
            FindViewById<TextView>(Resource.Id.close_ai_chat_button)!.Click += (_, _) => SetChatExpanded(false);

            _conversation = FindViewById<TextView>(Resource.Id.ai_chat_conversation);
            _chatInput = FindViewById<EditText>(Resource.Id.ai_chat_input);
            _sendButton = FindViewById<TextView>(Resource.Id.ai_chat_send_button);
        ...
        private void SendChatMessage(string? messageToRetry = null)
        {
            var message = messageToRetry ?? _chatInput?.Text?.Trim();
